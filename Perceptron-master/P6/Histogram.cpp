...
int Histogram::GenerateBuckets(int *_pixels, double _size) {
	
	_numPixels = _size;
	for (int pixel = 0; pixel < _size; pixel++)
...
void Histogram::Copy(Histogram _histogram) {
	_numPixels = _histogram._numPixels;
	for (unsigned int i = 0; i < 64; ++i) {
		_buckets[i] = _histogram._normalizedBuckets[i];
		_normalizedBuckets[i] = _histogram._normalizedBuckets[i];
	}
}
